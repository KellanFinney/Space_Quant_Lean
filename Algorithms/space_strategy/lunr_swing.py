...
class LUNRSwingStrategy(QCAlgorithm):
    """
    Intuitive Machines (LUNR) Swing Trading Strategy

    Signals:
    1. Technical: RSI, MACD, SMA crossover (20/50), Bollinger Bands
    2. Event-driven: Lunar mission launches, landings, NASA contract awards
    3. Position sizing: $1,000 account, long-only, 1-5 trades/week

    LUNR catalysts:
    - Lunar mission launches (IM-1, IM-2, IM-3) drive massive volume spikes
    - NASA CLPS contract awards signal revenue pipeline growth
    - Landing outcomes (success/partial/failure) create multi-day momentum
    - Earnings beats on backlog growth

    Rules:
    - Long only (no shorting)
    - Max 1 position at a time
    - Stop loss 7% (higher vol than RKLB)
    - Take profit 12-15%
    ...
    def Initialize(self):
        ...
        self.lunr = self.add_equity("LUNR", Resolution.Daily).Symbol
        self.event_data = self.add_data(LUNREvent, "LUNREVENTS", Resolution.Daily).Symbol

        # ---- Technical Indicators ----
        self.rsi = self.RSI(self.lunr, 14, MovingAverageType.Wilders, Resolution.Daily)
        self.macd = self.MACD(self.lunr, 12, 26, 9, MovingAverageType.Exponential, Resolution.Daily)
        self.sma_fast = self.SMA(self.lunr, 20, Resolution.Daily)
        self.sma_slow = self.SMA(self.lunr, 50, Resolution.Daily)
        self.bb = self.BB(self.lunr, 20, 2, MovingAverageType.Simple, Resolution.Daily)
        ...
        # ---- Event Tracking ----
        self.upcoming_event = False
        self.days_to_event = 999
    ...
    def OnData(self, data):
        ...
        if self.event_data in data:
            ...
            self.Log(f"LUNR EVENT: {event['Event']} ({event['Type']}) - {self.last_event_outcome}")

        if self.days_since_event < 999:
            self.days_since_event += 1

        if self.lunr not in data or data[self.lunr] is None:
            return

        price = self.Securities[self.lunr].Price
        if price <= 0:
            return

        if self.Portfolio[self.lunr].Invested:
        ...
        if signal_score >= 3:
            ...
            if shares > 0:
                self.MarketOrder(self.lunr, shares)
                self.entry_price = price
                self.entry_date = self.Time
                self.trades_this_week += 1
                self.Log(f"BUY {shares} LUNR @ ${price:.2f} | Signal: {signal_score} | RSI: {self.rsi.Current.Value:.1f} | MACD: {self.macd.Current.Value:.4f}")
    ...
    def CalculateSignalScore(self, price):
        ...
        # 6. Post-launch momentum (within 5 days of a launch)
        ...
        # 7. Post-landing momentum (lunar landing is huge catalyst)
        if self.days_since_event <= 5 and self.last_event_type == "Landing":
            score += 1
            reasons.append(f"Post-landing momentum ({self.days_since_event}d ago, {self.last_event_outcome})")

        # 8. NASA contract award momentum
    ...
    def ManagePosition(self, price):
        ...
        if pnl_pct <= -self.stop_loss_pct:
            self.Liquidate(self.lunr)
            self.Log(f"STOP LOSS: Sold LUNR @ ${price:.2f} | P&L: {pnl_pct*100:.1f}% | Held {days_held} days")
            self.ResetPosition()
            return

        if pnl_pct >= self.take_profit_pct:
            self.Liquidate(self.lunr)
            self.Log(f"TAKE PROFIT: Sold LUNR @ ${price:.2f} | P&L: {pnl_pct*100:.1f}% | Held {days_held} days")
            self.ResetPosition()
            return

        if days_held >= self.max_hold_days:
            self.Liquidate(self.lunr)
            self.Log(f"TIME STOP: Sold LUNR @ ${price:.2f} | P&L: {pnl_pct*100:.1f}% | Held {days_held} days")
            self.ResetPosition()
            return
        ...
        if pnl_pct > 0.06:
            ...
            if price <= trailing_stop:
                self.Liquidate(self.lunr)
                self.Log(f"TRAILING STOP: Sold LUNR @ ${price:.2f} | P&L: {pnl_pct*100:.1f}% | Held {days_held} days")
                self.ResetPosition()
                return
...
class LUNREvent(PythonData):
    """
    Custom data source: Intuitive Machines mission/contract events.
    """

    def GetSource(self, config, date, isLive):
        return SubscriptionDataSource(
            "/Lean/Data/custom/lunr_missions.csv",
