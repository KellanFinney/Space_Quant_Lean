...
class LMTSwingStrategy(QCAlgorithm):
    """
    Lockheed Martin (LMT) Space Division Swing Trading Strategy

    Signals:
    1. Technical: RSI, MACD, SMA crossover (20/50), Bollinger Bands
    2. Event-driven: Orion/Artemis milestones, SBIRS/Next-Gen OPIR satellite
                     launches, hypersonic program milestones, major defense contracts

    LMT space catalysts:
    - Orion spacecraft milestones (Artemis missions) validate deep-space business
    - SBIRS / Next-Gen OPIR satellite launches lock in multi-year MILSATCOM revenue
    - THAAD / missile defense contracts provide stable long-term backlog
    - Hypersonic vehicle tests (ARRW, HCSW) signal next-gen weapons revenue
    - Space fence and SSA contract awards from Space Force

    Rules:
    - Long only (no shorting)
    - Max 1 position at a time
    - Stop loss 4% (LMT is a large-cap defense; space events move stock modestly)
    - Take profit 8%
    ...
    def Initialize(self):
        ...
        self.lmt = self.add_equity("LMT", Resolution.Daily).Symbol
        self.event_data = self.add_data(LMTEvent, "LMTEVENTS", Resolution.Daily).Symbol

        # ---- Technical Indicators ----
        self.rsi = self.RSI(self.lmt, 14, MovingAverageType.Wilders, Resolution.Daily)
        self.macd = self.MACD(self.lmt, 12, 26, 9, MovingAverageType.Exponential, Resolution.Daily)
        self.sma_fast = self.SMA(self.lmt, 20, Resolution.Daily)
        self.sma_slow = self.SMA(self.lmt, 50, Resolution.Daily)
        self.bb = self.BB(self.lmt, 20, 2, MovingAverageType.Simple, Resolution.Daily)
    ...
    def OnData(self, data):
        ...
        if self.event_data in data:
            ...
            self.Log(f"LMT EVENT: {event['Event']} ({event['Type']}) - {self.last_event_outcome}")

        if self.days_since_event < 999:
            self.days_since_event += 1

        if self.lmt not in data or data[self.lmt] is None:
            return

        price = self.Securities[self.lmt].Price
        if price <= 0:
            return

        if self.Portfolio[self.lmt].Invested:
        ...
        if signal_score >= 3:
            ...
            if shares > 0:
                self.MarketOrder(self.lmt, shares)
                self.entry_price = price
                self.entry_date = self.Time
                self.trades_this_week += 1
                self.Log(f"BUY {shares} LMT @ ${price:.2f} | Signal: {signal_score} | RSI: {self.rsi.Current.Value:.1f} | MACD: {self.macd.Current.Value:.4f}")
    ...
    def CalculateSignalScore(self, price):
        ...
        # 6. Orion/Artemis program milestone
        if self.days_since_event <= 3 and self.last_event_type == "Artemis" and self.last_event_outcome == "Success":
            score += 1
            reasons.append(f"Orion/Artemis milestone ({self.days_since_event}d ago)")

        # 7. SBIRS / Next-Gen OPIR satellite launch success
        if self.days_since_event <= 3 and self.last_event_type == "Launch" and self.last_event_outcome == "Success":
            score += 1
            reasons.append(f"Satellite launch success ({self.days_since_event}d ago)")

        # 8. Major defense contract award (THAAD, hypersonic, Space Force)
    ...
    def ManagePosition(self, price):
        ...
        if pnl_pct <= -self.stop_loss_pct:
            self.Liquidate(self.lmt)
            self.Log(f"STOP LOSS: Sold LMT @ ${price:.2f} | P&L: {pnl_pct*100:.1f}% | Held {days_held} days")
            self.ResetPosition()
            return

        if pnl_pct >= self.take_profit_pct:
            self.Liquidate(self.lmt)
            self.Log(f"TAKE PROFIT: Sold LMT @ ${price:.2f} | P&L: {pnl_pct*100:.1f}% | Held {days_held} days")
            self.ResetPosition()
            return

        if days_held >= self.max_hold_days:
            self.Liquidate(self.lmt)
            self.Log(f"TIME STOP: Sold LMT @ ${price:.2f} | P&L: {pnl_pct*100:.1f}% | Held {days_held} days")
            self.ResetPosition()
            return
        ...
        if pnl_pct > 0.05:
            ...
            if price <= trailing_stop:
                self.Liquidate(self.lmt)
                self.Log(f"TRAILING STOP: Sold LMT @ ${price:.2f} | P&L: {pnl_pct*100:.1f}% | Held {days_held} days")
                self.ResetPosition()
                return
...
class LMTEvent(PythonData):
    """
    Custom data source: Lockheed Martin space division milestone events.

    CSV columns: Date, Event, Type, Outcome
    Types: Artemis | Launch | Contract
    Outcomes: Success | Partial | Failure
    """

    def GetSource(self, config, date, isLive):
        return SubscriptionDataSource(
            "/Lean/Data/custom/lmt_space_events.csv",
